...
/// <summary>
/// Classe héritant de "Feature" pour permettre le positionnement des objets de dimensions 1*3 16emes de mur.
/// </summary>
public class WallFeature1x3Manager : Feature {
	...
	public override Wall[] GetWallNeighbors(Wall wall, int part, int side)
	{
		...
		if (part == 3 || part == 7 || part == 11 || part == 15)
		{
			if (side == 1)
			{
				neighbors[1] = wall.neighbor.GetWall(wall.direction);
			}
			else
			{
				if (wall.dot.GetNeighbor(wall.direction.Opposite()))
				{
					neighbors[1] = wall.dot.GetNeighbor(wall.direction.Opposite()).GetWall(wall.direction);
				}
			}
		}
		else
	...
	public override int[] GetWallNeighborsParts(int part)
	{
		...
		if (part == 3 || part == 7 || part == 11 || part == 15)
		{
			neighborsParts[1] = part - 3;
		}
		else
		{
			neighborsParts[1] = part + 1;
		}
